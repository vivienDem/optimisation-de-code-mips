...
/** \brief returns the label of the function 
*/
	string get_name();
...
/**	\brief creates a new BB with the given start line, end line and branch line and its index, add it to the BB list of this. Branch line must be set to nullptr if the BB does not end with a branch (or jump or call instruction). Otherwise it must be set to the line of the branch.
*/
	void add_BB(Line *, Line*, Line*, int);

/**	\brief	Calculate the basics bolck of the function
*/
	void compute_basic_block();
...
private:
	...
	/** \brief true if loops has been computed
	 */
	bool loop_computed;
