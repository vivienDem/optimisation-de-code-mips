...
void Dfg::compute_critical_path(){
   list<Node_dfg*>::iterator it;
...
Node_dfg* Dfg::get_inst_for_delayed_slot(){
  ...
  if (itmp->is_syscall()) return nullptr; // on ne met rien après un syscall car pas de delayed slot dans Mars

  if (! (_delayed_slot.front()) -> get_instruction() -> is_nop())  // le delayed slot ne contient pas un nop
    return nullptr;
...
void  Dfg::scheduling(bool verbose){
   ...
   for(it=_roots.begin(); it!=_roots.end(); it++){
	 ...
	 if (is_ready2(succ2, &new_order,delayed_slot_inst)){
