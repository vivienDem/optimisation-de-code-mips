...
int main(int argc, char * argv[]){
	int nb_total_cycles_economises = 0;
	if (argc < 2) {
	...
	for(int i=0; i<  prog.nbr_func(); i++){
	  ...
	  delete(c);
	   ...
	   for(int j = 0;  j< functmp->nbr_BB(); j++){
	     ...
	     */
		 ...
		 nb_total_cycles_economises += nbcycles_sched_rename - nbcycles_sched_rename_sched;
	  ...
	  cout << "gain total renommage sched : " << nb_total_cycles_economises << endl;
