	...
	for(int i=0; i<  prog.nbr_func(); i++){
	  ...
	  delete(c);
	   ...
	   for(int j = 0;  j< functmp->nbr_BB(); j++){
	     ...
	     cout << " gain sched : " << nbcycles_base - nbcycles_sched << endl;
	    
	   }
	  name_f_cfg = "./tmp/" + name + "_cfg_func_" + functmp->get_name() +"_resched.dot";
	  c = new Cfg(functmp->get_BB(0), functmp->nbr_BB());
	  c->restitution(nullptr, name_f_cfg);
	  delete(c);
 
	}
	string name_file_out = name + "_resched.s";
